...
import 'logrecord_foratter.dart';
...
abstract class BaseLogSender extends BaseLogAppender {
  ...
  Future<void> _logEvent(LogEntry log) {
    print('base => logEvent function');
    _timer?.cancel();
    _timer = null;
    _logEvents.add(log);
    if (_logEvents.length > bufferSize) {
      print('base => if - _logEvents.length > bufferSize');
      _triggerSendLogEvents();
    } else {
      print('base => duration = 10s');
  ...
  Future<void> _triggerSendLogEvents() => Future(() {
    print('base => _triggerSendLogEvents');
...
class SimpleJobQueue {
  ...
  Future<int> triggerJobRuns() {
    print('base => triggerJobRuns');
    if (_currentStream != null) {
      print('Already running jobs. Ignoring trigger.');
      return Future.value(0);
    }
    print('Triggering Job Runs. ${_queue.length}');
...
//    final job = _queue.removeFirst();
    _currentStream = (() async* {
      ...
      if (_lastError!.difference(DateTime.now()).abs().compareTo(minWait) < 0) {
        print('There was an error. waiting at least $minWait');
        if (_queue.length > maxQueueSize) {
         print('clearing log buffer. ${_queue.length}');
          _queue.clear();
        }
      }
      return Future.value(null);
